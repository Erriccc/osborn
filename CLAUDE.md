...
- **Non-blocking `ask_agent`**: The `ask_agent` tool returns immediately ("Research started...") and runs Claude research in the background via `executeResearch()` (extracted function). Progress updates are queued in `activeResearch.pendingUpdates`, debounced by `scheduleResearchBatch()` (8s), and pushed to the unified `voiceQueue`. Final results are pushed via `queueVoiceInjection('[RESEARCH COMPLETE] ...')`. No manual task queuing — the Claude SDK handles sequential queries internally via session resume. When a new `ask_agent` call arrives while research is running, old listeners are cleaned up and the new task goes straight to the SDK. The SDK auto-resumes via `sessionId` — follow-up tasks inherit all prior research context.
