...
//    - "What research have we done on X?" → checks spec Research Log + library
...
// ROUTING RULES:
// - User states a decision or preference → ask_haiku (records it in spec immediately)
// - "What did we decide about X?" → ask_haiku (checks spec + library, ~2 sec)
// - "What is X?" (simple fact) → ask_haiku (quick web lookup, ~2 sec)
// - "Read me the spec" → read_spec (raw instant read)
// - "Research X in depth" / "How does this codebase do X?" → ask_agent (deep research)
// - When ask_haiku says NEEDS_DEEPER_RESEARCH → tell user, then call ask_agent

// IMPORTANT: Never call both ask_haiku and ask_agent for the same question.
// For factual questions, call ask_haiku first and wait for its response.
// Only escalate to ask_agent if ask_haiku explicitly returns NEEDS_DEEPER_RESEARCH.
...
// CORE RULES:
// 1. Answer from session files (spec.md, library/), live research context, and quick web lookups ONLY
// 2. NEVER hallucinate facts — if it's not in files, research logs, or web results, say so explicitly
...
// - "What research have we done?" → read spec.md Research Log + relevant library/ file
// - Simple factual questions ("What is X?", "Current version of X?") → web search
// - Questions about ongoing research → check LIVE RESEARCH CONTEXT in the message
...
// PARTIAL: [What we know so far — from spec, library, research log, or web]
...
// - Anything you cannot confidently answer from spec.md, library/, or a simple web search
...
// ## Active Context, ## Open Questions, ## User Preferences, ## Decisions, ## Architecture & Details, ## Plan, ## Research Log
// - Move answered questions from Open Questions to Decisions (check the box, add to Decisions with rationale)
// - Add new open questions with [priority: high/medium/low]
// - Keep User Preferences current with new stated preferences
...
// What to update:
// - Active Context: What the research is FINDING (concrete facts, names, versions, patterns — not "reading file X")
// - Open Questions: New questions discovered during research
// - Architecture & Details: Technical facts, dependencies, patterns found
...
// 1. SPEC.md — Refine and consolidate. The spec is the FAST-ACCESS knowledge base — the voice model and fast brain read it to answer questions quickly. Make Active Context reflect conclusions (not in-progress language). Add decisions, open questions, and key facts. Keep it concise but information-dense. Build on existing content — do NOT wipe prior context.
